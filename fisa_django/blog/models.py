...
#Tag table 작성 - pk는 자동생성됨
class Tag(models.Model) :
    tag_name = models.CharField(max_length=30, unique=True)
    # 짧은 라벨로서, 문자, 숫자, 밑줄 또는 하이픈만을 포함. 일반적을 URL에 사용.
...
class Post(models.Model): 
    ...
    # 외래키와 프라이머리키의 관계
    # CASCADE - User가 삭제되면 관련있는 Post 테이블의 모든 글을 삭제
...
class Comment(models.Model) :
    ...
    author = models.ForeignKey(User, on_delete=models.CASCADE) # 여러 사람이 하나의 댓글창을 공유
    content = models.TextField()
